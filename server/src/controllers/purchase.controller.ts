...
export const updatePurchase = catchAsync(async (req: Request, res: Response) => {
  ...
  const purchase = await purchaseService.updatePurchase(req.user?.localId, req.params.id, input);
...
export const listPurchases = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await purchaseService.listPurchases(req.user?.localId, query);
  res.json(serializeDecimals(result));
});

export const getPurchase = catchAsync(async (req: Request, res: Response) => {
  const purchase = await purchaseService.getPurchase(req.user?.localId, req.params.id);
  res.json({ purchase: serializeDecimals(purchase) });
});

export const receivePurchase = catchAsync(async (req: Request, res: Response) => {
  const purchase = await purchaseService.receivePurchase(req.user?.localId, req.params.id, req.user!.id);
  res.json({ purchase: serializeDecimals(purchase) });
});

export const cancelPurchase = catchAsync(async (req: Request, res: Response) => {
  const purchase = await purchaseService.cancelPurchase(req.user?.localId, req.params.id, req.user!.id);
  res.json({ purchase: serializeDecimals(purchase) });
});
